...
import { OfficerJenny } from "@/game/woody-plants";
...
describe("An Officer Jenny card", () => {
  it.each([
    [0, 0, 0, 0, 0],
    [2, 1, 0, 0, 0],
    [4, 1, 1, 0, 0],
    ...
    (expectedPoints, top, bottom, left, right) => {
      const woodyPlant = addDwellersToWoodyPlant(
        createAnyWoodyPlant(OfficerJenny),
      ...
      const points = OfficerJenny.score({ game, forest, woodyPlant });
